...
class DRQN(nn.Module):
    ...
    def forward(self, x, hidden):
        if not hidden:
            ...
            h0 = torch.zeros([self.layer_num, x.size(0), self.hidden_num])
            c0 = torch.zeros([self.layer_num, x.size(0), self.hidden_num])
            hidden = (h0, c0)
...
class ReplayBuffer:
    ...
    def sample(self):
        ...
        obss = torch.from_numpy(
            np.vstack([e.obs for e in experiences if e is not None])).float().to(device)
        actions = torch.from_numpy(
            np.vstack([e.action for e in experiences if e is not None])).long().to(device)
        rewards = torch.from_numpy(
            np.vstack([e.reward for e in experiences if e is not None])).float().to(device)
        next_obss = torch.from_numpy(
            np.vstack([e.next_obs for e in experiences if e is not None])).float().to(device)
...
class Agent(object):
    def __init__(self, action_size, observation_size, batch_size, seed):
        ...
        self.local_network.load_state_dict(self.target_net.state_dict())
